...
//fractional-timer by takamin(https://github.com/takamin)
...
let kijun=new Date("2012/6/2");//基準点 年から1000引いて月に3足して日に2を足す奴
let philKijun=new Date("2012/9/4");//精度の問題で+1000年する。 とりあえずグレゴリオ計算の流用
let stu_msec=809.26;//1stuは何ミリ秒？正解は越後製菓

function liToUTC(phil,stuj,snen,lies,ruke,stus){//リパラオネ→UTC  input:number output:Date
	let philDate=new Date();
	philDate.setFullYear(phil+1000);
	philDate.setMonth(stuj-1);
	philDate.setDate(lies);
	
	let philDiff=new BigNumber(philDate.getTime()).minus(philKijun.getTime()).dividedBy(24*60*60*1000).times(108000).plus(lies*4500+ruke*125+stus);
	let diff=philDiff.times(stu_msec);
	
	let date=new Date(kijun);
	date.setSeconds(diff.dividedBy(1000).toNumber());
	
	return date;
}

function raToUTC(phil,stuj,snen,zkd,zk,hk,stu2){//らねーめ☆ころん→UTC  input:number output:Date
	let philDate=new Date();
	philDate.setFullYear(phil+1000);
	philDate.setMonth(stuj-1);
	philDate.setDate(lies);
	
	let philDiff=new BigNumber(philDate.getTime()).minus(philKijun.getTime()).dividedBy(24*60*60*1000).times(108000).plus(zkd*10800+zk*1080+hk*108+stu2);
	let diff=philDiff.times(stu_msec);
...
function UTCToPh(iDate){//UTC→りぱらね  input:String or Date  output:Object
	let date=new BigNumber((typeof iDate=="Date"?iDate:new Date(iDate)).getTime());
	let diff=date.minus(kijun.getTime()).dividedBy(stu_msec);
	
	let difDay=diff.dividedBy(108000).floor();
	let difStu=diff.s==1?diff.modulo(108000):diff.modulo(108000).plus(108000);
	
	let lie=difStu.dividedBy(4500).floor();
	let ruk=difStu.modulo(4500).dividedBy(125).floor();
	let stu=difStu.modulo(125).floor();
	
	let zkd=difStu.dividedBy(10800).floor();
	let zk=difStu.modulo(10800).dividedBy(1080).floor();
	let hk=difStu.modulo(10800).modulo(1080).dividedBy(108).floor();
	let stu2=difStu.modulo(108).floor();
	
	let philDate=new Date(philKijun);
	philDate.setDate(philDate.getDate()+difDay.toNumber());
	
	
	return {
		phil:philDate.getFullYear()-1000,//ぴりふぃあー年
		stuj:philDate.getMonth()+1,//月
		snen:philDate.getDate(),//日
		
		lies:lie.toNumber(),//理時
		ruke:ruk.toNumber(),//理分
		stus:stu.toNumber(),//理秒
		
		zkd:zkd.toNumber(),//片時集
		zk:zk.toNumber(),//片時
		hk:hk.toNumber(),//挽時
		stu2:stu2.toNumber()//めっさ粉々やん時
	}
}

//以上ウンコード 以下は実用例(もウンコード)

FractionalTimer.setInterval(
	function(){
		let date=UTCToPh(new Date());
		document.getElementById("p").value=date.phil;
		document.getElementById("sj").value=date.stuj;
		document.getElementById("sn").value=date.snen;
		document.getElementById("l").value=date.lies;
		document.getElementById("r").value=date.ruke;
		document.getElementById("st").value=date.stus;
		document.getElementById("zd").value=date.zkd;
		document.getElementById("z").value=date.zk;
		document.getElementById("h").value=date.hk;
		document.getElementById("s2").value=date.stu2;
	},
	stu_msec
);